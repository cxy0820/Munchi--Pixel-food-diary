...
- `/api/remove-background` uses remove.bg by default. Set `BACKGROUND_PROVIDER=clipdrop` for Clipdrop, or install the local rembg requirements and set `BACKGROUND_PROVIDER=rembg` plus `ALLOW_LOCAL_REMBG=true` for local rembg.
